...
import { playbackPlan } from '../skills/q-flow/assets/viewer/src/playback.js';
...
const graph = {
  ...
  playback: { edgeIds: ['save'] }
};

test('English UI labels reach SVG, legend and playback without translating authored content', () => {
  ...
  assert.equal(playbackPlan(graph).description, 'Follow the authored walkthrough');
});
...
test('omitting locale preserves the existing Chinese rendering', () => {
  ...
  assert.equal(playbackPlan(legacy).description, '按已编排的演示顺序');
});
...
test('all seven README languages reach exports and formatted interface messages', () => {
  const serviceLabels = { en: 'Service', 'zh-CN': '服务', ja: 'サービス', ko: '서비스', de: 'Dienst', fr: 'Service', es: 'Servicio' };
  for (const [locale, service] of Object.entries(serviceLabels)) {
    ...
    const status = translate(locale, '{nodes} 个节点 · {edges} 条关系', { nodes: 6, edges: 5 });
    assert.ok(status.includes('6') && status.includes('5') && !status.includes('{'), locale);
