...
Flowchart main paths must run top-to-bottom, with left/right branch corridors and outside feedback routes; a column fold continues the main path at the top of the next column. Apply this rule to auto, preserve and image exports; omitting optional `primaryPath` must not let an unambiguous horizontal chain pass.
...
- Generation defaults to `--layout auto`; `--layout preserve` keeps geometry and applies the same strict gate. A finished diagram keeps its width/height ratio inside the band 1:1.6 to 1.6:1, so the graph's own shape decides between portrait and landscape. A layered result more than 10% outside that band folds: a top-down layout (architecture, flowchart, class, plain-chain state) that is too tall cuts its layers into columns that each keep reading downward and continue at the top of the next column; a left-to-right layout (er, deployment, usecase, dataflow) that is too wide cuts into rows that each keep reading rightward. The fewest columns or rows that pass the quality gate within 10% of the band win, a fold the gate rejects is skipped for the next one, and ranked layouts and branching state charts never fold. Readable point crossings are allowed, including nonplanar graphs. Prefer fewer repeated crossings; reject long shared segments and parallel channels closer than 24px. Keep at least 48px between nodes, 24px between labels and nodes/labels, 6px from unrelated lines, 32px inside boundaries and 48px between sibling groups. Do not shrink text, delete facts or use `--force` to bypass checks. A collection is one delivery: all views must pass before either output is replaced. Source, semantic, geometry and browser rendering evidence are separate.
