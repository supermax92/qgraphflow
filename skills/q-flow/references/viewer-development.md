...
- Use Radix Colors (MIT) as the shared palette: Slate for cool-neutral surfaces, Iris for core components and interactions, Cyan for data, and Orange for explicit failures. Keep copyright and license notices in distributed source, standalone HTML, and SVG; no runtime CDN or component-library dependency is required.
- Emphasize an actual business center with `business` or a case-insensitive `core`/`business` tag through a small Iris icon, an Iris 11 outline and Iris 12 text on a pale Iris fill. Explicit failures take precedence over core/module outline color; a module stripe retains identity. Initial/final symbols keep their notation. Decisions, choices, `alt` and FK references do not imply failure. Do not add literal color fields.
- Page, node drawings, MiniMap, Inspector dots, legends, and exports reuse `visual-style.js`. Ordinary cards use a low-saturation module or semantic tint; unclassified cards and dense ER/class rows use Slate 2; the light canvas is Slate 1 (`#fcfcfd`, near-white), the same step-1 rule as dark mode. Use corresponding dark scales rather than applying light colors unchanged in dark mode.
...
| Core surface / outline / text | Pale Iris tint / Iris 11 / Iris 12 (small icon plate Iris 3) |
| Interaction / directed motion | Iris 11 / actual relationship color |
| Data surface / outline / accent | Pale Cyan tint / Cyan 11 / Cyan 11 (ER header text uses Slate 11) |
| Failure surface / outline / accent | Pale Orange tint / Orange 11 (light darkened to `#c44b00` for 4.5:1 text contrast) |
| Decorative separator / meaningful edge | Slate 6 / Slate 11 |

- Ordinary node bodies use low-saturation fills: mix 12% identity/semantic color with the light surface, or 18% with the dark surface. Unclassified nodes remain neutral; initial/final symbols retain their notation. `moduleColorMap()` hashes the module name into the same bounded palette slot across diagrams, additions/removals and standalone exports. Slots can repeat; module labels remain authoritative. Module fills, outlines and small stripes carry identity; failure fills and outlines take precedence. ER/class tint their headers and keep dense rows neutral. Large groups, swimlanes and sequence fragments use a separate gauze palette with no outline. Do not recolor same-module neighbors merely to alternate fills.
- `groupAppearanceMap()` assigns nine independent region tones in deterministic reading order, avoiding equal consecutive siblings and parent/child fills. The approved demo hues supply a very pale fill plus a saturated header stripe of the same hue. Fills are precomposed over the theme canvas to prevent nested accumulation; render parents before children and keep every group below edges/cards. `groupFrameSvg()` shares borderless surfaces and small accents across page, minimap and SVG/PNG. Groups do not inherit module/status colors; names and sequence operators retain meaning when slots repeat. Headings and operators use neutral main ink. Verify edge contrast against group fills as well as the canvas.
- Build the legend from categories and line styles actually present; module-colored nodes use module entries instead of unused role-colored swatches. Use current theme tones. Test diagram text at ≥4.5:1 and meaningful outlines/lines at ≥3:1 against their actual backgrounds, including opacity. Preserve non-color symbols and text for grayscale/color-vision accessibility.
