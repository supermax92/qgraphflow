...
| Change directional edge motion | `features/useViewerController.js`, `DiagramCanvas.jsx` | Direction-only animation controlled by its switch and reduced-motion preference |
...
- Directed relationships show one subtle moving dash overlay from source to target by default; undirected relationships remain static. Preserve the solid/dashed evidence baseline beneath the overlay. User selection leaves these overlays moving. The separate `Edge animation` switch stops only ambient dashes; reduced motion stops all ambient motion.
...
- `effects.css` is a separate screen-only effect layer loaded after `styles.css`. In dark mode it adds a `filter: drop-shadow` glow along the SVG `.node-surface` in the node's module color when present, otherwise its role color, an accent glow for hover / selected, a wider selection halo (node 14px at .5, incident edges 14px at .3 so a selected moving dash keeps ≥ 60% of its reference contrast), a thicker glowing `.edge-flow`, and a 10% accent radial vignette from the top of the canvas. Light mode uses the same module color for a soft tinted shadow and keeps the canvas flat. The layer only references palette tokens, never a card `box-shadow` (sequence lifelines must keep `boxShadow === 'none'`), and never touches exports. `prefers-reduced-transparency: reduce` or `prefers-contrast: more` switches the whole layer off (filters `none`, halos and flow back to their `styles.css` values, vignette removed).
...
- Every fit (open, reset, view switch, `Fit canvas`, entering fullscreen) subtracts the chrome before computing the viewport: 52px toolbar + 12px at the top, the control row at the bottom, and on each side 304px + 24px when that floating panel is open or 24px when it is closed (`readingPadding()` in `useGraphLayout.js`, read from the canvas's `data-nav-open` / `data-drawer-open`). The padding object must use `px` strings; bare numbers are ratios.
- Opening a floating panel (navigation toggle, Inspector toggle, or `View details` on the quick-look card) over the selected node pans the canvas only far enough to uncover it with 12px clearance from the panel edge and never changes zoom; a node that is not covered does not move, and at 700px and below nothing moves (`useReveal.js`, called explicitly from those three actions, not from an effect). Directory and search selection keep centring on the node as before.
...
- The quick-look card tries right → left → below → above and takes the first placement that fits the canvas and covers no other node; if every fitting side overlaps a neighbour it takes the smallest overlap, measured with the card's real height. The card grows from the node's side (`is-flipped / is-below / is-above` set `transform-origin`).
