...
export function genericCard(node, x, y, fill, stroke, palette) {
  ...
  if (compact) return `<g>${paint(cardOutline(node, x, y), { fill, stroke })}<svg x="${x + 12}" y="${y + 10}" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="${tone}" stroke-width="1.5">${icon}</svg>${text(x + 33, y + 20, kindLabels[node.kind] ?? node.kind, 'stereotype', ' style="font-size:8px"')}${text(x + 10, y + 41, fit(node.label, width), 'compact-title')}${text(x + 10, y + 57, fit(node.subtitle ?? '', width), 'compact-body')}</g>`;
  ...
  return `<g filter="url(#node-shadow)">${device}${paint(cardOutline(node, x, y), { fill, stroke, 'stroke-width': 1 })}${container}<rect x="${x + 15}" y="${y + 13}" width="25" height="25" rx="6" fill="${core ? palette.hero : palette.group}"/><svg x="${x + 20}" y="${y + 18}" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="${tone}" stroke-width="1.5">${icon}</svg>${text(x + 47, y + 29, kindLabels[node.kind] ?? node.kind, 'stereotype')}${titleText}${subtitleText}</g>`;
}
