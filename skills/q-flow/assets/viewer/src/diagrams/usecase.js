...
function usecaseNode(node, x, y, fill, stroke, palette) {
  ...
  return `<g class="shape-label">${paint(outline(node, x, y), { fill, stroke, 'stroke-width': 1.2 })}${centeredTitle(cx, y + node.size.height / 2, node.label, node.size.width / Math.SQRT2 - 8, node.size.height / Math.SQRT2 - 8)}</g>`;
}
...
export default {
  ...
  render: usecaseNode, outline, anchor, undirected: ['association'],
  edgeLabel: edge => {
