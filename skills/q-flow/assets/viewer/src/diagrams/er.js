...
function erNode(node, x, y, fill, stroke, palette, locale) {
  ...
  return `<g filter="url(#node-shadow)">${paint(outline(node, x, y), { fill: palette.surface2, stroke, 'stroke-width': 1 })}<path d="M${x + 9} ${y}H${x + node.size.width - 9}Q${x + node.size.width} ${y} ${x + node.size.width} ${y + 9}V${y + header}H${x}V${y + 9}Q${x} ${y} ${x + 9} ${y}Z" fill="${fill}"/><path d="M${x} ${y + header}H${x + node.size.width}" stroke="${palette.rule}"/>${text(x + 13, y + header * .43, fit(node.label, node.size.width - 26), 'entity-title', ` style="font-size:${Math.min(TYPOGRAPHY.title, header * .38)}px;fill:${core ? palette.heroInk : palette.ink}"`)}${text(x + 13, y + header * .78, node.subtitle ?? translate(locale, '实体'), 'entity-meta', ` style="font-size:${Math.min(TYPOGRAPHY.small, header * .22)}px;fill:${core ? palette.heroInk : palette.data}"`)}${text(x + node.size.width - 13, y + header * .78, String(node.fields.length).padStart(2, '0'), 'entity-meta', ` text-anchor="end" style="font-size:${Math.min(TYPOGRAPHY.small, header * .22)}px;fill:${core ? palette.heroInk : palette.data}"`)}${rows}</g>`;
}
