import { centeredTitle, rectangle, paint } from './drawing.js';
...
function dataflowNode(node, x, y, fill, stroke) {
  ...
  return `<g class="shape-label">${paint(outline(node, x, y), { fill, stroke, 'stroke-width': 1.2 })}${inner}${centeredTitle(x + node.size.width / 2, y + node.size.height / 2, node.label, node.size.width - 32)}</g>`;
}
