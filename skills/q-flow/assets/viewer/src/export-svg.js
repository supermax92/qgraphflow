...
function edgeMarker(edge, type, target, moduleColors, source) {
  const { end } = edgeMarkers(edge, type);
  if (!end) return '';
  if (end !== 'arrow') return ` marker-end="url(#${end})"`;
  const marker = edge.kind === 'failure' ? 'arrow-warn' : edge.kind === 'success' ? 'arrow-ok' : moduleColors?.has(edge.module ?? source?.module ?? target?.module) ? 'arrow-module' : dataKinds.has(target?.kind) ? 'arrow-data' : 'arrow';
...
function renderGroup(group, offsetX, offsetY, palette) {
  ...
  const label = fit(group.label, group.size.width - (['alt', 'opt', 'loop'].includes(group.kind) ? 88 : 32));
  return `<g><title>${escapeXml(group.label)}</title><rect x="${x}" y="${y}" width="${group.size.width}" height="${group.size.height}" rx="14" fill="${fill}" stroke="${stroke}" stroke-width="1"${group.kind === 'external' ? ' stroke-dasharray="6 6"' : ''}/>${text(x + 16, y + 23, label, 'group')}${['alt', 'opt', 'loop'].includes(group.kind) ? text(x + group.size.width - 16, y + 25, group.kind, 'group-kind', ' text-anchor="end"') : ''}</g>`;
}

function renderEdge(edge, route, type, offsetX, offsetY, palette, target, moduleColors, source) {
  const stroke = edgeColor(edge, target, palette, moduleColors, source);
  ...
  return `<g><path d="${pathFromPoints(route.points, offsetX, offsetY)}" fill="none" stroke="${stroke}" stroke-width="1.5"${dash}${edgeMarker(edge, type, target, moduleColors, source)}${edgeStartMarker(edge, type)}/>${cardinalities}${background}${label ? route.labelLines.map((line, index) => text(labelX, labelY - labelSize.height / 2 + 3 + TYPOGRAPHY.body + index * TYPOGRAPHY.edgeLineHeight, line, 'edge', ' text-anchor="middle"')).join('') : ''}</g>`;
}
...
export function createDiagramSvg(graph, theme = 'light', moduleColors) {
  ...
  const edges = graph.edges.map(edge => renderEdge(edge, routes.get(edge.id), type, offsetX, offsetY, palette, nodeById.get(edge.target), moduleColors, nodeById.get(edge.source))).join('');
  const nodes = graph.nodes.map(node => renderNode(node, type, offsetX, offsetY, palette, graph.meta.locale, moduleColors)).join('');
  const boardX = 24;
  const boardY = header;
