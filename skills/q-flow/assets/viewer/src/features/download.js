...
export async function saveGraphJson(input, locale, setStatus, pageName = decodeURIComponent(window.location?.pathname?.split('/').pop() || 'index.html')) {
  ...
  try {
    if (typeof window.showDirectoryPicker === 'function') {
      ...
      setStatus(t('Graph JSON saved'));
    } else {
      downloadBlob(new Blob([contents], { type: 'application/json;charset=utf-8' }), 'graph.json');
      setStatus(t('Graph JSON downloaded; keep the file to preserve edits'));
    }
  } catch (error) {
    if (writable) await writable.abort().catch(() => {});
    setStatus(t(error.name === 'AbortError' ? 'Save cancelled; edits remain in this page'
      : error.name === 'NotFoundError' ? 'Choose the folder containing this page ({page}); edits remain in this page'
      : 'Save failed; edits remain in this page', { page: pageName }));
  }
}
...
export async function verifyRenderedSvg(svg, graph) {
  ...
  if (parsed.querySelector('parsererror')) throw qualityFailure(graph, 'rendering', 'The exported SVG could not be parsed');
  ...
  try {
    ...
    if (!Number.isFinite(offsetX) || !Number.isFinite(offsetY)) throw new Error('The exported SVG has no geometry offset');
    ...
    const checkLabel = (elements, value, safe, id, label = 'Edges') => {
    ...
    for (const group of graph.groups ?? []) {
      ...
      checkLabel([...(element?.querySelectorAll(':scope > text.group') ?? [])], group.label, shifted(heading), group.id, 'group');
      ...
      for (const [kind, boxes] of [['guard', fragment?.guards], ['body', fragment?.bodies]]) for (const box of boxes ?? []) {
        ...
        checkLabel(elements, box.lines.join(''), shifted(box), `${group.id}/${box.operandId}/${kind}`, 'fragment');
      }
    }
    for (const edge of graph.edges) {
      ...
      if (!line) throw new Error(`Export is missing edge ${edge.id}`);
      ...
      if (getDiagram(type).cardinalities) for (const [side, point, neighbor] of [['source', route.points[0], route.points[1]], ['target', route.points.at(-1), route.points.at(-2)]]) {
        ...
        if (!path || path.getAttribute('d') !== expected.path || Boolean(circle) !== Boolean(expected.circle)
          || expected.circle && Object.entries(expected.circle).some(([key, value]) => Number(circle.getAttribute(key)) !== value)) throw new Error(`Cardinality mark missing or mismatched: ${edge.id}/${side}`);
        const actual = boxOf(mark), safe = shifted(expected.bounds);
        if (!inside(actual, safe) || !inside(safe, actual) || !inside(actual, view)) throw new Error(`Cardinality mark is out of bounds: ${edge.id}/${side}`);
      }
      for (const [side, expected] of Object.entries(edgeMarkers(edge, type))) {
        ...
        if (!expected || !id || !(id === expected || expected === 'arrow' && /^arrow-(module|warn|ok|data)$/.test(id))) throw new Error(`Edge marker mismatch: ${edge.id}/${side}`);
        const marker = root.querySelector(`marker[id="${CSS.escape(id)}"]`);
        if (!marker) throw new Error(`Edge marker missing: ${edge.id}/${side}/${id}`);
        if (!markerGeometry.has(id)) {
          ...
          try {
            ...
            if (!box.width || !box.height) throw new Error(`Edge marker is empty: ${edge.id}/${id}`);
        ...
        if (!(width > 0 && height > 0 && vb.width > 0 && vb.height > 0) || !inside(box, vb)) throw new Error(`Edge marker is clipped: ${edge.id}/${id}`);
  ...
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      try {
        ...
        if (!width || !height || width > 32767 || height > 32767 || width * height > 64_000_000) throw new Error('Image exceeds the PNG export limit; export SVG instead');
        const canvas = document.createElement('canvas'); canvas.width = width; canvas.height = height;
        const context = canvas.getContext('2d');
        if (!context) throw new Error('The browser could not create the PNG canvas');
        ...
        if (!probe) throw new Error('The browser could not inspect the PNG canvas');
...
export async function downloadDiagram(graph, theme, format, setStatus, moduleColors) {
    ...
    try {
      ...
      setStatus(t('{format} downloaded', { format: format.toUpperCase() }));
